...
class Action
  ...
  def initialize
    ...
    $call1 = Gosu::Sample.new("vis/call1.wav")
    @tune0 = Gosu::Song.new("vis/tune0.wav")
    @tune1 = Gosu::Song.new("vis/tune1.wav")
    @tune2 = Gosu::Song.new("vis/tune2.wav")
    @tune3 = Gosu::Song.new("vis/tune3.wav")
    @tune4 = Gosu::Song.new("vis/tune4.wav")
    @cursor = 4
  end
  ...
  if Gosu.button_down? Gosu::KB_UP and !@pressed and $sing
    $sing = false
    @tune.stop
    $tune_num += 1 if $tune_num < 5
    $tune_num = 0 if $tune_num == 5
    @beep.play
    @pressed = true
  elsif not Gosu.button_down? Gosu::KB_RIGHT and not Gosu.button_down? Gosu::KB_DOWN and not Gosu.button_down? Gosu::KB_LEFT
    @pressed = nil
 ...
 def cannoteat
  ...
  if Gosu.button_down? Gosu::KB_DOWN and !@pressed and $feed and $move
    ...
    @eat.play if $food > 0 and not ($hungry < -1 and $food > 0 and not ($id == 3 and @cursor == 12))
    @cannot.play if $food == 0 or ($hungry < -1 and $food > 0) and not ($id == 3 and @cursor == 12)
    @pressed = true
   elsif not Gosu.button_down? Gosu::KB_RIGHT and not Gosu.button_down? Gosu::KB_DOWN and not Gosu.button_down? Gosu::KB_LEFT
   @pressed = nil
  end
 ...
 if !$light
   if !$sick and $sleepstate == 1 and !$muchvariable
    ...
    if $id == 0 or $id == 2
       ...
       $talk1 = "Please stay with me for a while..." if $sleepy > 3 and $id == 2
    elsif $id == 1 or $id == 5
      if $sleepy < 5
       unless $sad > 4
         $state = 7 
...
if $sleepstate == 8 and $lifecounter == $catsleep
 ...
 if Gosu.button_down? Gosu::KB_DOWN and !@pressed and @cursor == 18 and $move
  ...
  @cursor = 1
  $state = 9
  @tune = @tune0 if $tune_num == 0
  @tune = @tune1 if $tune_num == 1
  @tune = @tune2 if $tune_num == 2
  @tune = @tune3 if $tune_num == 3
  @tune = @tune4 if $tune_num == 4
  $wait = $lifecounter + 1050 if $tune_num == 0
  $wait = $lifecounter + 1250 if $tune_num == 1
  $wait = $lifecounter + 1170 if $tune_num == 2
  $wait = $lifecounter + 1000 if $tune_num == 3
  $wait = $lifecounter + 1300 if $tune_num == 4
  @tune.play
  elsif $id == 3
  $cantdo = true
  @decide.play
 ...
 def update
   ...
   if $testmode
     if (Gosu.button_down? Gosu::KB_T or $readytotalk) and !@pressed2 and $talkdelay <= $lifecounter and $move and !$play and !$sing and $rpsls == 0 and !$feed and !$dead and !$start and not ($id == 3 and $light and $sleepy > 4) and !$dream and !$asleep and not ($id == 4 and $sad > 4 and $light) and not ($id != 3 and !$light and $sleepy > 4) and !$conversation and $maybreak == 0 and !$sleepdeprivation and $sleepstate != 4 and !$sneeze
       unless (($negativetalk_incrementation > 5 or $negative_incrementation > 5) and $id == 2) or $dialogue == 0
        $call.play if !$broken_deer
        $call1.play if $broken_deer
       end
       $light = true
       $sleepstate = nil
       $conversation = true
   ...
   if !$testmode
     if $readytotalk and $talkdelay <= $lifecounter and $move and !$play and !$sing and $rpsls == 0 and !$feed and !$dead and !$start and not ($id == 3 and $light and $sleepy > 4) and !$dream and !$asleep and not ($id == 4 and $sad > 4 and $light) and not ($id != 3 and !$light and $sleepy > 4) and !$conversation and $maybreak == 0 and !$sleepdeprivation and $sleepstate != 4 and !$sneeze
       unless (($negativetalk_incrementation > 5 or $negative_incrementation > 5) and $id == 2) or $dialogue == 0
        $call.play if !$broken_deer
        $call1.play if $broken_deer
       end
       ...
       $d_number = 1
          ...
          when 11
            ...
            if $d_number == 1 and $lifecounter == $wait
              $endoftalk = $lifecounter + 350
              $state = 11
              $talk = "A bunch of fireflies was sitting"
              $talk1 = "on it."
              $talk2 = nil
            end
          when 12
          ...
          when 2
            ...
            if $d_number == 2 or $d_number == 3 or $d_number == 4
              if Gosu.button_down? Gosu::KB_A or Gosu.button_down? Gosu::KB_B or Gosu.button_down? Gosu::KB_C or Gosu.button_down? Gosu::KB_D or Gosu.button_down? Gosu::KB_E or Gosu.button_down? Gosu::KB_F or 
                ...
                Gosu.button_down? Gosu::KB_TAB or Gosu.button_down? Gosu::KB_LEFT or Gosu.button_down? Gosu::KB_RIGHT or Gosu.button_down? Gosu::KB_DOWN or Gosu.button_down? Gosu::KB_UP or Gosu.button_down? Gosu::KB_RETURN
                $wait = $lifecounter + 200
                $state = 12
                $talk = "Cooooooool!"
          ...
          when 4
            if $d_number == 0
              $wait = $lifecounter + 400
              $state = 12
              $talk = "Oh, don't think  that I'm obsessed"
              $talk1 = "with death..."
              $d_number = 1
            end
          ...
          when 12
            ...
            if $d_number == 1 and $lifecounter == $wait
              ...
              $talk1 = "If I didn't,  I would have lost"
              $talk2 = "a few lives."
              $d_number = 2
            end
        ...
        when 1
          ...
          when 8
            ...
            if $d_number == 1 and $lifecounter == $wait
              ...
              $talk1 = "constant  worry that it may"
          ...
          when 16
            ...
            if $d_number == 1
              if Gosu.button_down? Gosu::KB_Y
                $endoftalk = $lifecounter + 300
                $state = 12
                $talk = "Great! Thank you for your answer."
                $talk1 = nil
                $respect += 1
                $sad -= 1
                negativetalk_decrementation
          ...
          when 18
            ...
            if $d_number == 3 and $lifecounter == $wait
              $wait = $lifecounter + 400
              $state = 15
              $talk = "They are very convenient because"
              $talk1 = "they help you  to feel exactly"
              $talk2 = "what you want to feel."
              $d_number = 4
            end
        ...
        when 2
          ...
          when 3
            ...
            if $d_number == 1 and $lifecounter == $wait
              $wait = $lifecounter + 600
              $state = 18
              $talk = "May you please answer me  at least"
              $talk1 = "sometimes just to let me  know"
              $talk2 = "that you hear me..."
        ...
        when 5
          ...
          when 16
            if $d_number == 0
              ...
              $talk1 = "If it would not trouble you, "
              $talk2 = "of course..."
              $d_number = 1
            end
          ...
          when 17
            ...
            if $d_number == 1 and $lifecounter == $wait
              $wait = $lifecounter + 300
              $state = 11
              $talk = "I'd been a seal pup before I died..."
              $talk1 = nil
              $d_number = 2
            end
        ...
        when 4
          ...
          when 3
            ...
            if $d_number == 2 and $lifecounter == $wait
              ...
              $talk1 = "a game out of taking care of"
...
class Stats
  ...
  def update
   if !$dead and !$broken_deer
    ...
    $state = 6 if $move
    $sad += 1 if $lifecounter % 1500 == 1 and $testmode
    $sad += 1 if $lifecounter % 12000 == 11000 and !$testmode
    $hungry += 1 if $lifecounter % 500 == 1 and $id != 4 and $testmode
    $hungry += 1 if $lifecounter % 10500 == 10445 and $id != 4 and !$testmode
    if $id == 4 and $light
      $hungry -= 1 if $lifecounter % 1500 == 1 and $maybreak == 0 and $testmode
      $hungry -= 1 if $lifecounter % 4000 == 3850 and $maybreak == 0 and !$testmode
    elsif $id == 4 and !$light
      $hungry += 1 if $lifecounter % 500 == 1 and $testmode
      $hungry += 1 if $lifecounter % 4500 == 4255 and !$testmode
    end
    $sleepy += 1 if $lifecounter % 3500 == 1 and $id != 4 and $testmode
    $sleepy += 1 if $lifecounter % 25000 == 24360 and $id != 4 and !$testmode
    #$sleepy += 1 if $lifecounter % 200 == 1 and $id != 4
    elsif $sick and $asleep
    $state = 5
    $sad += 1 if $lifecounter % 2000 == 1 and $testmode
    $sad += 1 if $lifecounter % 10000 == 9999 and !$testmode
    $hungry += 1 if $lifecounter % 4500 == 1 and $testmode
    $hungry += 1 if $lifecounter % 18000 == 17950 and !$testmode
    $sleepy -= 1 if $lifecounter % 3000 == 2000 and $id != 4 and $testmode
    $sleepy -= 1 if $lifecounter % 20000 == 19600 and $id != 4 and !$testmode
    #$sleepy -= 1 if $lifecounter % 200 == 1 and $id != 4
    elsif !$sick and $asleep
    $state = 5
    $sad -= 1 if $lifecounter % 500 == 1 and $id == 4 and $testmode
    $sad -= 1 if $lifecounter % 7000 == 6999 and $id == 4 and !$testmode
    $hungry += 1 if $lifecounter % 5000 == 1 and $id != 4 and $testmode
    $hungry += 1 if $lifecounter % 25000 == 24555 and $id != 4 and !$testmode
    $hungry -= 1 if $lifecounter % 1500 == 1 and $id == 4 and $testmode
    $hungry -= 1 if $lifecounter % 3500 == 3400 and $id == 4 and !$testmode
    $sleepy -= 1 if $lifecounter % 2000 == 1999 and $id != 4 and $testmode
    $sleepy -= 1 if $lifecounter % 10000 == 9999 and $id != 4 and !$testmode
   ...
   if !$conversation and !$ending and !$sneeze and $maydie < $lifecounter and (($hungry > 8 and $id != 5 and $id != 4 and $sick) or $dead_deer)
    death
   end
...
class Tamago < Gosu::Window
  ...
  def draw
    ...
    #bitmap($sneezerand.to_s, 60, 13, 0) if $testmode
